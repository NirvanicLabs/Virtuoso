...
{
    self.strContent = nil;
    self.strContent = [[NSMutableString alloc]init];
    ...
    CGPDFScannerRef scanner = CGPDFScannerCreate(content, self.operatorTable, (__bridge void * _Nullable)(self));
...
- (void)detector:(StringDetector *)detector foundString:(NSString *)needle unicodeString:(NSString *)str
{
...
void didScanSpace(float value, Scanner *scanner)
{
    ...
    if (abs(value) >= [scanner.currentRenderingState.font widthOfSpace])
    {
        ...
        [scanner.strContent appendString:@" "];
...
void didScanString(CGPDFStringRef pdfString, Scanner *scanner)
{
    ...
    [scanner.strContent appendString:string];
}
...
@synthesize documentURL, keyword, stringDetector, fontCollection, renderingStateStack, currentSelection, selections, str,rawTextContent,strContent;
@end
