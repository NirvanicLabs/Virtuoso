...
- (void)didFindNeedle:(NSString *)str;
...
- (void)didFindNeedle:(NSString *)str
{
    if ([delegate respondsToSelector:@selector(detector:foundString:unicodeString:)])
    {
        // Tell the delegate where the needle was found
        [delegate detector:self foundString:self.keyword unicodeString:str];
    }
}
    ...
    for (int i = 0; i < [unicodeString length]; i++)
    {
        ...
        if ([self append:needleString isLast:&isLast font:font])
        {
            ...
            if (isLast)
            {
                ...
                [self didFindNeedle:unicodeString];
                [self reset];
            }
        }
        else
